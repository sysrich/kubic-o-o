...
But when it comes to condensing that upstream code into an integrated product, we're going to continue using the [openSUSE Build Service](https://build.opensuse.org) as our platform of choice for building packages, container, and VM images.  
Contributing to Kubic for the most part will say the same, following the regular [openSUSE Factory Process](https://en.opensuse.org/openSUSE:How_to_contribute_to_Factory).  
...
Everyone can see what we're working on, but for the moment if you'd like to join the Board, please reach out to get permission.
